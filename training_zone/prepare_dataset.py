...
def prepare_dataset(s3_client):
    ...
    page_iterator = paginator.paginate(Bucket=os.getenv("TRAINING_ZONE_BUCKET"), Prefix="image/")
        ...
        if "Contents" in page:
            for obj in page["Contents"]:
                ...
                if game_id in game_descriptions:
                    dataset.append(
                        {"image_path": key, "description": game_descriptions[game_id], "game_id": game_id}
                    )
                else:
    ...
    for split_name, split_df in zip(["train", "val", "test"], [train_df, val_df, test_df]):
        ...
        try:
            s3_client.put_object(
                Bucket=os.getenv("TRAINING_ZONE_BUCKET"), Key=f"data_splits/{split_name}.csv", Body=csv_obj
            )
            logging.info(f"Saved {split_name} split with {len(split_df)} records.")
        except Exception:
            logging.exception(f"Error saving {split_name} split to MinIO.")
    logging.info("Data splits saved to MinIO.")
