...
def format_to_json(s3_client, source_key, type_folder):
    try:
        ...
        obj = s3_client.get_object(Bucket=LANDING_ZONE_BUCKET, Key=source_key)
        ...
        new_key = f'{JSON_SUB_BUCKET}/{type_folder}/{base_name}.{TARGET_TAB_FORMAT}'
...
def move_to_formatted_zone(s3_client, key, type):
    try:
        ...
        new_key = f'{JSON_SUB_BUCKET}/{type}/{base_name}'
...
def format_json_objects(s3_client, objects, type):
    ...
    if is_empty(s3_client, FORMATTED_ZONE_BUCKET, f'{JSON_SUB_BUCKET}/{type}/'):
        curr_format = last_key.split('.')[-1].lower()
        if curr_format != TARGET_TAB_FORMAT:
    ...
    # otherwise, check if the date of files in landing and formatted is the same
        ...
        try:
            prefix = f'{JSON_SUB_BUCKET}/{type}/'
            formatted_zone_objects  = s3_client.list_objects_v2(Bucket=FORMATTED_ZONE_BUCKET, Prefix=prefix)
            ...
            if formatted_file_date != landing_file_date:
                ...
                s3_client.delete_object(Bucket=FORMATTED_ZONE_BUCKET, Key=old_key)
...
def main():
    ...
    try:
        ...
        json_path = f'{PERSISTENT_SUB_BUCKET}/json/'
        steam_objects = s3_client.list_objects_v2(Bucket=LANDING_ZONE_BUCKET, Prefix=json_path+'steam/')
        steamspy_objects = s3_client.list_objects_v2(Bucket=LANDING_ZONE_BUCKET, Prefix=json_path+'steamspy/')
