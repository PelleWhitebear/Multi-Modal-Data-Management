...
#     "lora_matrices": ["q_proj", "v_proj"], # we can try adding "k_proj", "out_proj" or even MLP layers ("gate_proj", "up_proj")
#     "lora_dropout": 0.1,
...
def main():
    ...
    # Load model (in fp16) and processor
    logging.info(f"Loading model {CONFIG['model_id']} on {CONFIG['device']}...")
    model = CLIPModel.from_pretrained(CONFIG['model_id'], torch_dtype=torch.float16).to(CONFIG['device'])
